...
fun listdivider(input: List<Int>): List<Int> {
    ...
    //1. 10,4,2,1
    val leftDivided = listdivider(leftList)
    //1. 7,3,13,5
    val rightDivided = listdivider(rightList)
    //한번 나눈 list를 병합
//    println("$leftDivided $rightDivided")
    return upmerge(leftDivided, rightDivided)
}
...
//남은거 list3에 3번에 저장
//나머지 반복
fun upmerge(left: List<Int>, right: List<Int>): List<Int> {
    ...
    //남아있는 leftList의 값들을 추가한다.
    //1. 0 < 1 true
    while (leftIndex < left.size) {
        //1. mergeSortList[4, 10], false
        mergeSortList.add(left[leftIndex])
        leftIndex++
    }
    //남아있는 rightList 값들을 추가한다
    //1. 1 < 1 false
    while (rightIndex < right.size) {
        //1. mergeSortList[4,10]
        mergeSortList.add(right[rightIndex])
        rightIndex++
    }
...
fun listdivider2(input: List<Int>): List<Int> {
    if (input.size < 2) {
        return input
    }
    val mid = input.size / 2
    ...
    val leftDivided = listdivider2(leftList)
    val rightDivided = listdivider2(rightList)
//    println("$leftDivided $rightDivided")
    return downmerge(leftDivided, rightDivided)
}

fun downmerge(left: List<Int>, right: List<Int>): List<Int> {
...
println(listdivider2(mutableListOf<Int>(10, 4, 2, 1, 7, 3, 13, 5)))
